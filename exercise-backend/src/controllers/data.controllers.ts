...
export class DataController {
  ...
  getDataCategory(req: Request, res: Response) {
    const data: IData[] = JSON.parse(
      fs.readFileSync("./db/data.json", "utf-8")
    );
    ...
    res.status(200).send({ total_income, total_expense, income, expense });
  }
  getDataId(req: Request, res: Response) {
    ...
    res.status(200).send({ data: resultId });
  }
  ...
  getTotalByCategory(req: Request, res: Response) {
    ...
    res.status(200).send({ total_income, total_expense });
